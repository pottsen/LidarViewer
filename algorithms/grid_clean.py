...
class Grid():
    ...
    def make_grid_by_cell(self, size_of_cells):
        ...
        self.base_max_x = np.max(base_x)
        self.base_min_x = np.min(base_x)
        ...
        self.base_max_y = np.max(base_y)
        self.base_min_y = np.min(base_y)
        ...
        self.base_max_z = np.max(base_z)
        self.base_min_z = np.min(base_z)
        ...
        delta_x = abs(self.base_max_x - self.base_min_x)
        delta_y = abs(self.base_max_y - self.base_min_y)
    ...
    def add_points_to_grid(self, flag):
        ...
        if flag == "base":
            ...
            for i in range(len(self.base_file.points)):
                ...
                grid_x = math.floor((self.base_x[i]-self.base_min_x)/self.cell_size)
                grid_y = math.floor((self.base_y[i]-self.base_min_y)/self.cell_size)
                # try:
            ...
            for i in range(len(self.snow_file.points)):
                if self.snow_x[i] > self.base_max_x or self.snow_x[i] < self.base_min_x or self.snow_y[i] > self.base_max_y or self.snow_y[i] < self.base_min_y:
                    print("Snow point out of grid range")
                else:
                    if (i % 1000000) == 0:
                        print(i, " of ", len(self.base_file.points), " snow points added to grid")
                    grid_x = math.floor((self.snow_x[i]-self.base_min_x)/self.cell_size)
                    grid_y = math.floor((self.snow_y[i]-self.base_min_y)/self.cell_size)
                    # try:
    ...
    def regrid_point_cloud(self):
        ...
        for i in range(len(self.snow_xyz)):
            if self.snow_matched_xyz[i][0] > self.base_max_x or self.snow_matched_xyz[i][0] < self.base_min_x or self.snow_matched_xyz[i][1] > self.base_max_y or self.snow_matched_xyz[i][1] < self.base_min_y:
                print("Snow point out of grid range")
            else:
                if (i % 1000000) == 0:
                    print(i, " of ", len(self.base_file.points), " snow points added to grid")
                grid_x = math.floor((self.snow_matched_xyz[i][0]-self.base_min_x)/self.cell_size)
                grid_y = math.floor((self.snow_matched_xyz[i][1]-self.base_min_y)/self.cell_size)
                # try:
