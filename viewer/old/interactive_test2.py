...
class DemoScene(QtWidgets.QWidget):
    def __init__(self, keys='interactive'):#, points, color):
        ...
        avg_z = np.average(z)
        print(np.average(z))
        self.data = np.transpose(np.stack((x, y, z)))
        ...
        self.facecolor = np.ones((len(self.data), 4), dtype=np.float)
        self.ptsize = 3
